...
In this project, we use Intel SGX DCAP to support remote attestation to provide a certification that indicates this service is securely running within an enclave on an Intel® SGX enabled platform. As [Example of Webhook Return Data](#example-of-webhook-return-data) shown above, there is a field `tee_report` which is the remote attestation certification (the report) generated by the Intel® SGX enabled platform on which the service is currently running.

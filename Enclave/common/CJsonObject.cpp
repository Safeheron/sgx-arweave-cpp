...
bool CJsonObject::Add(const std::string& key_str, const std::string& data_str) {
    ...
    if (cjson_check_return_ == nullptr) {
        ERROR("Request ID: %s, Add %s failed.", request_id, key_str.c_str());
        return false;
    }
    return true;
}
...
bool CJsonObject::Add(const std::string& key_str, CJsonObject& cjson_object) {
    if (cJSON_IsArray(cjson_object.GainCJsonData()) || cJSON_IsObject(cjson_object.GainCJsonData())) {
        ...
        ERROR("Request ID: %s, Key: %s invalid input.", request_id, key_str.c_str());
        return false;
    }
...
bool CJsonObject::Add(const std::string& data_str) {
    ...
    if (!cjson_ok_) {
        ERROR("Request ID: %s, Add string to array failed.", request_id);
        return false;
    }
    return true;
}
...
bool CJsonObject::Add(CJsonObject& cjson_object) {
    ...
    if (!cjson_ok_) {
        ERROR("Request ID: %s, Add object to array failed.", request_id);
        return false;
    }
    cjson_object.PointToNull();
    return true;
...
bool CJsonObject::Parse(const std::string& data_str) {
    ...
    if (cjson_data_ == nullptr) {
        ERROR("Request ID: %s, Parse cjson data failed.", request_id);
        return false;
    }
    return true;
}
...
bool CJsonObject::ToString(std::string& cjson_to_str) {
    ...
    if (cjson_to_str.empty()) {
        ERROR("Request ID: %s, Change cjson to string failed.", request_id);
        return false;
    }
    free(temp);
    return true;
...
bool CJsonObject::CreateObject() {
    ...
    if (cjson_data_ == nullptr) {
        ERROR("Request ID: %s, Create object failed.", request_id);
        return false;
    }
    return true;
}
...
bool CJsonObject::CreateArray() {
    ...
    if (cjson_data_ == nullptr) {
        ERROR("Request ID: %s, Create array failed.", request_id);
        return false;
    }
    return true;
}
...
bool CJsonObject::GetValueString(const std::string& key_str, std::string& value_str) {
    ...
    if (value_str.empty()) {
        ERROR("Request ID: %s, Get %s failed.", request_id, key_str.c_str());
        return false;
    }
    return true;
}
...
bool CJsonObject::GetArray(const std::string& key_str, std::vector<std::string>& pubkey_list) {
    ...
    if (0 == array_size) {
        ERROR("Request ID: %s, pubkey_list_size error", request_id);
        return false;
    }

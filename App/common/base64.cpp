...
void base64_encode( const uint8_t* input, size_t size, std::string & b64 )
{
    ...
    encoded = b64_encode ( input, size );
    if ( !encoded ) {
        ERROR( "b64_encode() return null!" );
        return;
    }
...
void base64_decode( const std::string b64, std::string & output )
{
    ...
    decoded = b64_decode_ex( b64.c_str(), b64.length(), &decoded_len );
    if ( !decoded ) {
        ERROR( "b64_encode() return null!" );
        return;
    }
    output.assign( (char*)decoded, decoded_len );
